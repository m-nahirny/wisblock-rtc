...
uint8_t readFromRegister(uint8_t registerAddress){
    ...
    i2c_write_blocking(i2c_rak, RV3028_ADDRESS, &registerAddress, 1, true);
    int numRead = i2c_read_blocking(i2c_rak, RV3028_ADDRESS, &result, 1, false);
    if (numRead == PICO_ERROR_GENERIC)
    {
        uart_puts(uart1, "No device present");
    }
    return result;
}
...
void writeToRegister(uint8_t registerAddress, uint8_t value){
    ...
    buf[1] = value;
    i2c_write_blocking(i2c_rak, RV3028_ADDRESS, buf, 2, false);
}
...
void writeToRegisters(uint8_t startAddress, uint8_t *values, uint8_t length){
    ...
    sprintf(print_buf, "Writing to registers starting at %d ", startAddress);
    uart_puts(uart1, print_buf);
    // Append register address to front of data packet
    msg[0] = startAddress;
    for (int i = 0; i < length; i++) {
        msg[i + 1] = values[i];
        sprintf(print_buf, "%02x", values[i]);
        uart_puts(uart1, print_buf);
    }

    // Write data to register(s) over I2C
    i2c_write_blocking(i2c_rak, RV3028_ADDRESS, msg, (length + 1), false);
